...
import { makeMockedService } from 'shared/utils/tests/integrations/mockServiceMethod';
import { getDiffValueBgClassname, getDiffValueBorderClassname } from '../../shared/DiffHighlight/DiffHighlight';
...
const mockedExperimentRunsDataService = makeMockedService({ path: 'services/experimentRuns/ExperimentRunsDataService', service: ExperimentRunsDataService });
...
const renderComponent = async ({
  ...
  mockedExperimentRunsDataService.mockMethod('loadExperimentRunsByIds', jest.fn(async () => [experimentRun1, experimentRun2]));
...
const makeMockExperimentRun = ({
  ...
  const ownerExpRun = new User({
    id: owner.id,
    email: `${owner.id}@gmail.com`,
    username: owner.username,
  });
  experimentRun.owner = ownerExpRun;
  experimentRun.ownerId = owner.id;
...
const updateExperimentRun = (
  ...
  if (owner) {
    ...
    newExperimentRun.owner = ownerExpRun;
    newExperimentRun.id = owner.id;
  }
  if (shortExperiment) {
...
const checkBgHighlighting = (
  ...
  return component
    .getDOMNode()
    .classList.contains(
      getDiffValueBgClassname(modelType, true) || '',
    );
};
...
const checkBorderHighlighting = (
  ...
  return component
    .getDOMNode()
    .classList.contains(
      getDiffValueBorderClassname(modelType, true) || ''
    );
};
...
const getDisplayedCodeVersion = (
  ...
  }
  {
    ...
    return gitCodeVersionsInfo.length === 1
      ...
      : gitCodeVersionsInfo[modelType === 1 ? 0 : 1];
  }
};
...
describe('components', () => {
  describe('CompareModels', () => {
    const experimentRun1 = makeMockExperimentRun({
      ...
      owner: { id: 'owner-id-1', username: 'owner-1' },
    });
    ...
    const experimentRun2 = makeMockExperimentRun({
      ...
      owner: { id: 'owner-id-2', username: 'owner-2' },
    });
    ...
    describe('property "id"', () => {
      ...
      it('should highlight background ids values', async () => {
        ...
        expect(checkBgHighlighting(1, firstIdElem)).toBe(true);
        expect(checkBgHighlighting(2, secondIdElem)).toBe(true);
      });
    });
    ...
    describe('property "owner"', () => {
      it('should render comparing row for property', async () => {
        ...
        expect(
          findByDataTestAttribute('property-name-ownerId', component).length
        ).toBe(1);
        expect(
          findByDataTestAttribute('property-name-ownerId', component).text()
        ).toBe('Owner');
      });
      ...
      it('should render correct value for the first experiment run', async () => {
        ...
        expect(
          findByDataTestAttribute('property-value-ownerId', component)
            .at(0)
            .text()
        ).toBe(experimentRun1.owner.username);
      });
      ...
      it('should render correct value for the second experiment run', async () => {
        ...
        expect(
          findByDataTestAttribute('property-value-ownerId', component)
            .at(1)
            .text()
        ).toBe(experimentRun2.owner.username);
      });

      it('should highlight background owners values if they are not equal', async () => {
        const { component } = await renderComponent({
          experimentRun1: updateExperimentRun(
            { owner: { username: 'owner-1', id: 'owner-id-1' } },
            experimentRun1
          ),
          experimentRun2: updateExperimentRun(
            { owner: { username: 'owner-2', id: 'owner-id-2' } },
            experimentRun2
          ),
        });
        const firstIdElem = findByDataTestAttribute(
      ...
      it('should not highlight background owners values if they are equal', async () => {
        ...
        expect(checkBgHighlighting(1, firstIdElem)).toBe(false);
        expect(checkBgHighlighting(2, secondIdElem)).toBe(false);
      });
    });
    ...
    describe('property "Metrics"', () => {
      ...
      it('should render correct value for the first experiment run', async () => {
        ...
        const displayedMetrics = getDisplayedMetrics(1, component);
      ...
      it('should render correct value for the second experiment run', async () => {
        ...
        const displayedMetrics = getDisplayedMetrics(2, component);
      ...
      it('should highlight background of metric value if metrics with the same key have not the same value or if metric with key is not existed', async () => {
        ...
        const displayedMetrics1 = getDisplayedMetrics(1, component);
        const displayedMetrics2 = getDisplayedMetrics(2, component);
        expectedResult.forEach(({ key, isDiff }) => {
          ...
          if (targetDisplayedMetric1) {
            expect(
              checkBgHighlighting(1, targetDisplayedMetric1.value.elem)
            ).toBe(isDiff);
          }
          if (targetDisplayedMetric2) {
            expect(
              checkBgHighlighting(2, targetDisplayedMetric2.value.elem)
            ).toBe(isDiff);
          }
        });
      });
    ...
    describe('property "Attributes"', () => {
      ...
      it('should highlight border of attribute button if attributes with the same key have not the same value or if attribute with key is not existed', async () => {
        ...
        const displayedAttributes1 = getDisplayedAttributes(1, component);
    ...
    describe('property "Artifacts"', () => {
      ...
      it('should highlight border of artifact button if artifacts with the same keys have not the same type or path or linkedArtifactId or if artifact with key is not existed', async () => {
        ...
        for (const testCase of testCases) {
          ...
          expect(
            checkBorderHighlighting(1, displayedArtifact1.button)
          ).toBe(testCase.isDiff);
          if (displayedArtifact2) {
            expect(
              checkBorderHighlighting(2, displayedArtifact2.button)
            ).toBe(testCase.isDiff);
          }
        }
      });
      ...
      describe('when code versions type are "git"', () => {
        ...
        it('should higlight border of git buttons if exec path or remote repo url or commit hash is different', async () => {
          ...
          for (const testCase of testCases) {
            ...
            ([1, 2] as const).forEach(modelType => {
        ...
        it('should not display "Compare Code" button if remote repo urls are not equals and commit hashs are not existed', async () => {
          const testCases: Array<{
            gitCodeVersion1Data: IGitCodeVersionData;
            gitCodeVersion2Data: IGitCodeVersionData;
          }> = [
              {
                gitCodeVersion1Data: {
                  remoteRepoUrl: remoteRepoUrl1,
          ...
          for (const testCase of testCases) {
            ...
            openCompareCodeVersionsPopup('git', 1, component);
